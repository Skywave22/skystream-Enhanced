/// Catalog / meta objects returned by Stremio add-ons, plus adapters to the
/// app's own [MultimediaItem] so add-on content flows through existing UI.
library;
...
class AddonMetaPreview {
    ...
    for (final key in const ['genres', 'genre']) {
      ...
      if (raw is List) {
        for (final entry in raw) {
          if (entry is String && entry.isNotEmpty) genres.add(entry);
        }
      }
    }
  ...
  bool get isSeries => type == 'series' || type == 'anime' || type == 'tv';
  ...
  MultimediaItem toMultimediaItem() {
    return MultimediaItem(
      ...
      imdbId: id.startsWith('tt') ? id.split(':').first : null,
    );
  }
}

/// A single episode/video inside an add-on meta object.
class AddonVideo {
  final String id;
  final String title;
  final int? season;
...
/// Full meta object (`/meta/{type}/{id}.json`).
class AddonMeta extends AddonMetaPreview {
  ...
  factory AddonMeta.fromJson(
    ...
    if (rawCast is List) {
      for (final entry in rawCast) {
        if (entry is String && entry.isNotEmpty) cast.add(entry);
      }
    }
    ...
    return AddonMeta(
      ...
      videos: videos,
      cast: cast,
      director: (json['director'] is List && (json['director'] as List).isNotEmpty)
          ? (json['director'] as List).first.toString()
  ...
  MultimediaItem toMultimediaItem() {
    ...
    return base.copyWith(
      episodes: videos.isEmpty
          ? null
          : videos.map((v) => v.toEpisode()).toList(),
    );
  }
}

/// Helpers for the Stremio content-id grammar (`tt0944947:1:5`).
class StremioId {
  const StremioId._();

  static String forEpisode(String baseId, int season, int episode) =>
      '$baseId:$season:$episode';

  static String base(String id) => id.split(':').first;

  static bool isImdb(String id) => id.startsWith('tt');
}
